import type { IconDefinition, IconTarget, NotablePlugin } from "notable-plugin-api";

const EMOJI: Record<string, IconDefinition> = {
...
const plugin: NotablePlugin = {
  onload(api) {
    ...
    const choose = async (kind: IconTarget["kind"], path: string) => {
